...
public class Employees_and_ProjectsDAO extends SessionUI {
    ...
    public Employees_and_Projects getByEmployee(Integer id) {
        SessionUI sessionUI = new SessionUI();
        Session session = createSessionFactory().openSession();
        session.beginTransaction();
        String sql = "SELECT * FROM Employees_and_Projects WHERE employeeId = :id";
        Query query = session.createSQLQuery(sql).addEntity(Employees_and_Projects.class);
        query.setParameter("id", id);
        Employees_and_Projects employees_and_projects = (Employees_and_Projects) query.getSingleResult();
        session.getTransaction().commit();
        session.close();
        sessionUI.getSessionFactory().close();
